...
app.put('/utilisateurs/modificationProfil', (request, response) => {
  ...
  jwt.verify(token, secretKey, (error, decoded) => {
    ...
    db.query('update utilisateurs set utilisateurs_nom = ?, utilisateurs_prenom = ?, utilisateurs_adresse_email = ?, utilisateurs_numero_de_telephone = ? WHERE utilisateurs_id = ?', [utilisateurs_nom, utilisateurs_prenom, utilisateurs_adresse_email, utilisateurs_numero_de_telephone, id], (error) => {
      if (error) {
        console.log(error)
      } else {
...
app.post('/utilisateurs/inscription', (request, response) => {
  ...
  bcrypt.genSalt(saltRound, (error, salt) => {
    ...
    bcrypt.hash(utilisateurs_mot_de_passe, salt, (error, hash) => {
      ...
      db.query('insert into utilisateurs (utilisateurs_nom, utilisateurs_prenom, utilisateurs_adresse_email, utilisateurs_mot_de_passe, utilisateurs_numero_de_telephone) values (?, ?, ?, ?, ?)', [utilisateurs_nom, utilisateurs_prenom, utilisateurs_adresse_email, hash, utilisateurs_numero_de_telephone], (error) => {
        if (error) {
          console.log(error)
        } else {
    ...
    db.query('select * from utilisateurs where utilisateurs_id = ?', id,(error, result) => {
      ...
      bcrypt.compare(oldPassword, user.utilisateurs_mot_de_passe, (error, match) => {
        ...
        bcrypt.hash(newPassword, saltRound, (error, newHash) => {
          ...
          db.query('update utilisateurs set utilisateurs_mot_de_passe = ? where utilisateurs_id = ?', [newHash, id], (error) => {
            if (error) {
              console.log(error)
            } else {
